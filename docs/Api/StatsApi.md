...
# **getStats_0**
> getStats_0($resource, $type, $id, $start_date, $end_date, $timezone, $group_by_resource)
...
try {
    $api_instance->getStats_0($resource, $type, $id, $start_date, $end_date, $timezone, $group_by_resource);
} catch (Exception $e) {
    echo 'Exception when calling StatsApi->getStats_0: ', $e->getMessage(), PHP_EOL;
}
?>
```
